...
JVM loads User.class into the Method Area (if not already loaded).

JVM allocates memory in the HEAP for the User object.

JVM stores a reference to this object in the STACK variable u.

JVM initializes the object (default values + constructor).

Object becomes reachable as long as at least one reference points to it.
